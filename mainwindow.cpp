...
void MainWindow::on_pushButtonFindDrivers_clicked()
{
    ...
    foreach (const QStorageInfo &storage, storageList) { // add all drivers
        ui->comboBoxDrivers->addItem(storage.rootPath());
    }

    for (int i = 0; i < 3; i++) // delete MY drivers
    {
        ui->comboBoxDrivers->removeItem(0);
    }
}
...
{
    //QString openFilePath = ui->lineEditOpenFileLocation->text();
    ...
    ui->labelBaudRate->setText("9.5 Mbit/s");
...
void MainWindow::on_pushButtonShowData_clicked()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) { // try to open file
        if(file.size() == 0) { // if file open but it is empty
            ...
            //fileData = in.readAll(); // global data for NextSymbol
